...
## Task 15 addendum: closing a coverage gap left by task 13

Task 13's original fixture set only ever wires `lotl-valid.xml` (and its `-tampered`/
`-stale-next-update` siblings) to `tl-national-a-valid.xml` and `tl-national-b-valid.xml` —
both plain, both `granted`, both valid. `tl-national-tampered.xml` (ZZ),
`tl-national-three-statuses.xml` (CC) and `tl-national-stale-next-update.xml` (DD) were built
but never referenced by any LOTL fixture. That is fine for `VerifyTslFixtures`'s signature-only
smoke check, but it means **AC-02** ("one national list tampered, the LOTL and the other lists
are fine, sync continues normally") and **AC-03 exercised through the real DSS pipeline**
(rather than only at unit level) had no fixture LOTL to sync against — `lotl-tampered.xml`
corrupts the *LOTL's own* signature (that is `ES-02`'s scenario, a different case: no national
list is derived at all), it does not point at a tampered *national* list.

Task 15 (the container test, `TrustListSyncIT`) adds two files to close this, without modifying
any file task 13 committed:

| File | Purpose |
|------|---------|
| `lotl-valid-mixed.xml` (+ its `templates/` source) | A LOTL, signed the same way as every other LOTL fixture, whose own signature is valid but whose four pointers mix `tl-national-b-valid.xml` (BB, valid — proves "other lists are fine"), `tl-national-tampered.xml` (ZZ, ES-02/AC-02's rejection path), `tl-national-three-statuses.xml` (CC, AC-03 through a real sync) and `http://tsl-fixtures-unreachable/tl-does-not-exist.xml` — a hostname deliberately never registered as a network alias in the container test, giving EC-01 a clean, deterministic DNS failure instead of a stopped-container timing race. |
| `keystore/official-test-truststore.p12` (password `dss-password`) | The store `DssTrustListJobConfig.officialSigningCertificateSource()` loads in the container test to verify these fixtures — i.e. it plays the role of the real, production `oj-keystore.p12` (task 7), but for the test CA. Contains exactly one entry, `tsl-test-signing-ca.cer` (already committed by task 13) imported as a trusted certificate — **not** the same file as `tsl-test-signing-keystore.p12` above, which holds the *private key* used to sign fixture content and is protected with a different password (`tsl-test-password`). `DssTrustListJobConfig` hardcodes `dss-password` as the keystore password (it is not a Spring property), so this store's password must match that constant exactly, independent of the signing keystore's own password. |

Both files are regenerated by the same `regenerate-fixtures.sh` used for every other fixture
here; see the script for the exact `keytool -importcert` invocation for the truststore.
